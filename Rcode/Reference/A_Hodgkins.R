...
getwd()
setwd("C:/Users/u0152843/OneDrive - KU Leuven/My Drive/KULeuven_PhD/C1_RE_PEAT/Mid_Infrared/Data/")
...
n <- 19  # number of points to differentiate over; must be odd
m <- (n-1)/2
...
for(i in seq_along(data)) {
  ...
  w1.result <- minima(xydata[x %in% 939:960,], default=980)
  #w1.result <- minima(xydata[x %in% 950:1020,], default=980)
  ...
  w2.result <- minima(xydata[x %in% 1150:1198,], default=1185) # higher w2 option
...
as.data.frame(norm.silicate780)

sum(as.data.frame(area_and_silicate)$norm.silicate780>0.00005)

View(area_and_silicate)
# Normalize to area ----
norm.Ap <- Ap/area
...
############################################################################
#Peaek_data_calculation
############################################################################

norm.Acorr <- rownames_to_column(norm.Acorr,var="ID")



Outcomes_Hodgkins <- norm.Acorr %>% 
  mutate(sum_arom = arom15+arom16,
         sum_aliph = aliph28+aliph29,
         HI_arom1 = arom15/carb,
         HI_arom2 = arom16/carb,
         HI_acid = acids/carb,
         HI_aliph1 = aliph28/carb,
         HI_aliph2 = aliph29/carb,
         HI_arom = sum_arom/carb,
         HI_aliph = sum_aliph/carb,
         carbohydrate_est = 49204*carb-1.7606,
         aromatic_est = 42332*sum_arom +7.3476)




str(Outcomes_Hodgkins)

#################################################################################
#write.csv(Outcomes_Hodgkins,"C:/Users/u0152843/OneDrive - KU Leuven/My Drive/KULeuven_PhD/C1_RE_PEAT/MIR/Data/Outcomes_Hodgkins.csv")
...
#extract_the_label
Outcomes_Hodgkins$Site <-  sub("^([^_]*).*$", "\\1", Outcomes_Hodgkins$ID)
Outcomes_Hodgkins$Treatment <- sub("^([^_]*_)([^_]*)(_).*$", "\\2", Outcomes_Hodgkins$ID)
Outcomes_Hodgkins$Depth <- sub("^([^_]*_)([^_]*_)([^_]*).*$", "\\3",Outcomes_Hodgkins$ID)

###Explore data##############


ggboxplot(Outcomes_Hodgkins, x ="Depth" ,y ="HI_aliph1" ,color = "Treatment")
ggboxplot(Outcomes_Hodgkins, x ="Depth" ,y ="HI_aliph2" ,color = "Treatment")
ggboxplot(Outcomes_Hodgkins, x ="Depth" ,y ="HI_arom1" ,color = "Treatment")
ggboxplot(Outcomes_Hodgkins, x ="Depth" ,y ="HI_arom2" ,color = "Treatment")
ggboxplot(Outcomes_Hodgkins, x ="Depth" ,y ="HI_acid" ,color = "Treatment")


ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="HI_aliph2" ,color = "Depth")
ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="HI_acid" ,color = "Depth")
ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="HI_arom2" ,color = "Depth")
ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="HI_arom1" ,color = "Depth")


ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="carb" ,color = "Depth")
ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="carb" ,color = "Depth")
ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="arom15" ,color = "Depth")
ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="arom16" ,color = "Depth")
ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="aliph28" ,color = "Depth")
ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="aliph29" ,color = "Depth")
ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="carbohydrate_est" ,color = "Depth")
ggboxplot(Outcomes_Hodgkins, x ="Treatment" ,y ="aromatic_est" ,color = "Depth")
