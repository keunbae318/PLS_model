...
for(mirs in 1:length(Compounds_group)){
  ...
  validation.dat$RPD <- data_import[data_import$AIC == sort(data_import$AIC)[1],]$RPD
...
ggplot(final_new_data,aes(x = meas, y=predicted))+
  geom_point()+
  geom_abline(color="grey",size = 1,linetype="dashed")+
  theme_bw()+
  facet_wrap(Index ~ ., scales = "free",  ncol= 5)+
  geom_text(aes(x = Inf, y = -Inf, label = paste("Best_fit =", model_name)), vjust = -6.5, hjust = 1, size = 4)+
  geom_text(aes(x = Inf, y = -Inf, label = paste("Rsq =", Rsquared)), vjust = -5, hjust = 1, size = 4)+
  geom_text(aes(x = Inf, y = -Inf, label = paste("RMSE =", RMSD)), vjust = -3.5, hjust = 1, size = 4)+
  geom_text(aes(x = Inf, y = -Inf, label = paste("AIC =", AIC)), vjust = -2, hjust = 1, size = 4)+
  geom_text(aes(x = Inf, y = -Inf, label = paste("RPD =", RPD)), vjust = -0.5, hjust = 1, size = 4)+
...
for(mirs in 1:length(Compounds_group)){
  ...
  Index <- Compounds_group[mirs]
  data_import <- read.table(paste0(Index,"39/MIR",Index,"diffFactors.txt"),header = T)
  data_import <- data_import[-(161:180),]
  #average calculation
  average_group <- data_import %>% 
    summarise(Compounds_group = Index,
              avg_Rsquared = mean(Rsquared)
              )
  
 
  all_new_data[[mirs]] <- average_group
}
all_new_data
