...
for (w in 1: length(Compounds_group)){
  ...
  for (p in 1: number_of_preproc){
    ...
    for (i in 1: totalSpectraCalib){
      ...
      calibData$measured[i]  =  measured[[Index]][row]
    }
...
View(regResults)
regResults
#write.csv(regResults, "C:/Users/u0152843/OneDrive - KU Leuven/My Drive/KULeuven_PhD/C1_RE_PEAT/Mid_Infrared/PLS_model/LOO_result_V2.csv")
PLS.result.dat <- read.csv("PLS_model/LOO_result_V2.csv")

lowest_aic_models <- regResults %>%
  group_by(compound) %>%
  slice(which.min(AIC))
