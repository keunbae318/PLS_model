...
#---------------------------------------------------------
# Compute the averages spectrum based on the 5 replicates 
...
SpecAver <- function (x, repNum, ID, thr = 0.03){
  ...
  siteName <- matrix(NA, n_samples, 1)
  # Create the empty matrix with the number of the replicate
  pitRep <- matrix(NA, n_samples, 1)
  ...
  for (i in seq(from = 1, to = n_samples)){
    ...
    siteName[i,1] <-  sub("^([^_]*).*$", "\\1", ID[nameRow,1])
    pitRep[i,1] <- sub("^([^_]*_)([^_]*)(_).*$", "\\2", ID[nameRow,1])
    depth[i,1] <- sub("^([^_]*_)([^_]*_)([^_]*).*$", "\\3",ID[nameRow,1])
    newID[i,1] <- paste(siteName[i,1],"_",pitRep[i,1],"_",depth[i,1],sep = "") 
    
  }
  return (list(SpecMeans = rep_means, SdRep = rep_sds, Check = meanStd, site = siteName, pitRep = pitRep, depth = depth, ID = newID))
}
